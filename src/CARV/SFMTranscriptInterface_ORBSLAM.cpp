   ...
   try{
        ...
        for (int i = 0; i < kf->mLines3D.rows;i++) {
            ...
            for (int j=0;j<=10;j=j+5)
            {
                ...
                matNewPoint(2) = kf->mLines3D.at<float>(i,2) + j*z1;
...
void SFMTranscriptInterface_ORBSLAM::addPlaneKeyFrameInsertionEntry(ORB_SLAM2::KeyFrame *kf){
    try{
        std::stringstream ssTmp;
        dlovi::Matrix matNewCam(3, 1);
        //dlovi::Matrix matNewPoint(3, 1);
         ...
         {  
            if(!kf->mValidPlane[i])
                {continue;}
                    ...
                    float x1 = (kf->mLines3D.at<float>(p,3) - kf->mLines3D.at<float>(p,0))/10;
                    ...
                    for (int j=0;j<=10;j=j+2)
                    {
                        ...
                        matNewPoint(2) = kf->mLines3D.at<float>(p,2) + j*z1;

                        ssTmp << "new point: [" << matNewPoint(0) << "; " << matNewPoint(1) << "; " << matNewPoint(2) << "]";
                        ssTmp << ", " << nCamIndex;
                        m_SFMTranscript.addLine(ssTmp.str()); ssTmp.str("");
                    }
                    x1 = (kf->mLines3D.at<float>(q,3) - kf->mLines3D.at<float>(q,0))/10;
                    ...
                    for (int j=0;j<=10;j=j+2)
                    {
                        ...
                        matNewPoint(2) = kf->mLines3D.at<float>(q,2) + j*z1;
            ...
            if (counter<4)
            {
            for (int i = 0; i < kf->mLines3D.rows; i=i+2) {
            float x1 = (kf->mLines3D.at<float>(i,3) - kf->mLines3D.at<float>(i,0))/10;
            float y1 = (kf->mLines3D.at<float>(i,4) - kf->mLines3D.at<float>(i,1))/10;
            float z1 = (kf->mLines3D.at<float>(i,5) - kf->mLines3D.at<float>(i,2))/10;
            for (int j=0;j<=10;j=j+5)
